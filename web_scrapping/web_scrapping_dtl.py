...
def scrap_dtl(start:int, end:int, filename:str):
    '''Scrapping data details (bahan, bumbu, metode)'''
    link_list = []
    ingredients_list = []
    spice_list = []
    method_list = []

    data = pd.read_csv("data\data_awal.csv")
    data.drop("Unnamed: 0", axis=1, inplace=True)

    # for link in data['link'][4000:5000]:
    for link in data['link'][start:end]:

        # website = 'https://resepmamiku.com/ikan-senangin-cabai-hijau-dita_dwistyani'
        html = urlopen(link)
